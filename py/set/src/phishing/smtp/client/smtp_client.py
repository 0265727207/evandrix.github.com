...
if os.path.isfile("src/program_junk/template.pdf"): 
	if os.path.isfile("src/program_junk/template.rar"):
		if os.path.isfile("src/program_junk/template.zip"):
		        print """Multiple payloads were detected. Which one do you want?\n\n"""
	        	choose_payload=raw_input("1. PDF Payload\n2. VBS Payload\n3. Zipfile Payload\n\nEnter choice: ")
		...
		if not os.path.isfile("src/program_junk/template.doc"):
			if not os.path.isfile("src/program_junk/template.rtf"):
				print "No previous payload created."
	       			file_format=raw_input("Enter the file you want to use as an attachment: ")
	        		if not os.path.isfile("%s" % (file_format)):
	        			print "\nFILE NOT FOUND. Try Again.\n"
	        			file_format=raw_input("Enter the file you want to use as an attachment: ")
	        			print ("Sorry hoss, that was twice, check the filepath and try again. Exiting..\n\n")
	        			sys.exit()
print """
...
"""
filename1=raw_input("Enter your choice (enter for default): ")
if filename1 == '1' or filename1 == '':
   print "   Keeping the filename and moving on."
if filename1 == '2':
	filename1=raw_input("   Enter the new filename: ")
	...
	print "   Filename changed, moving on..."
...
if relay == '1':
   ...
   user1 = user
   pwd = getpass.getpass("   Enter your password for %s (it will not be displayed back to you): " % (email_provider))
   #smtp = ("smtp.gmail.com")
   #port = ("587")
...
def mail(to, subject, text, attach, prioflag1, prioflag2):
	...
	if sendmail == 1: thread.start_new_thread(mailServer.sendmail,(user, to, msg.as_string()))    
if option1 == '1':
		...
		if not os.path.isfile("src/program_junk/payload.options"):
			if not os.path.isfile("src/program_junk/meta_config"):
				if not os.path.isfile("src/program_junk/unc_config"):
					print "Sorry you did not generate your payload through SET, this option is not supported."
