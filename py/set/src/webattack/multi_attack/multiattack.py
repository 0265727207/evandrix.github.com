...
def return_menu():
	raw_input("\nOption added. Press " + bcolors.RED+"{return}" + bcolors.ENDC+" to add or prepare your next attack.")
...
# turning flag on
def flag_on(vector):
	print "\nTurning the %s Attack Vector to " % (vector) + bcolors.GREEN + "ON" + bcolors.ENDC

# turning flag off
def flag_off(vector):
        print "\nTurning the %s Attack Vector to " % (vector) + bcolors.RED + "OFF" + bcolors.ENDC
...
subprocess.Popen("rm -rf src/webattack/web_clone/site/template 1> /dev/null 2> /dev/null;mkdir src/webattack/web_clone/site/template 1> /dev/null 2> /dev/null", shell=True).wait()
...
""")
	print "1. The Java Applet Attack Method" + toggleflag_java
	print "2. The Metasploit Browser Exploit Method" + toggleflag_meta
