...
print (core.bcolors.YELLOW + "  Web Server Launched. Welcome to the SET MLTM.")
print (core.bcolors.BLUE + "***************************************************")
print (core.bcolors.BLUE + "Man Left in the Middle Attack brought to you by:\nKyle Osborn - kyle@kyleosborn.com" + core.bcolors.ENDC)
print "\nStarting server on %s:%s..." % (bind,port)
try:
	serv = HTTPServer((bind, int(port)), RequestHandler)
	print core.bcolors.GREEN + "[*] Server has started" + core.bcolors.ENDC
	serv.serve_forever()
except Exception, e:
	print e
