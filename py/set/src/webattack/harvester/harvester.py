...
try:
	...
	print "Python OpenSSL wasn't detected, if your running debian-base apt-get install python-openssl"
	sys.exit()
	...
	if ssl_flag=='true':
		...
		for line in fileopen:
			...
			if match:
				...
				import ssl
				subprocess.Popen("cp src/program_junk/CA/*.pem src/program_junk 1> /dev/null 2>/dev/null", shell=True).wait()
				# remove old junk we dont need anymore
				subprocess.Popen("rm -rf src/program_junk/CA 1> /dev/null 2> /dev/null;cp *.pem src/program_junk 1> /dev/null 2> /dev/null", shell=True).wait()
			...
			for line in fileopen:
				...
				if match:
					...
					if os.path.isfile(pem_client):
						subprocess.Popen("cp %s src/program_junk/newcert.pem 1> /dev/null 2> /dev/null" % (pem_client), shell=True).wait()
						sys.exit()					
				...
				if match2:
					...
					if os.path.isfile(pem_server):
						subprocess.Popen("cp %s src/program_junk/newreq.pem 1> /dev/null 2> /dev/null" % (pem_server), shell=True).wait()
...
class SETHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
	...
	# import proper style css files here
		...
		if self.path == "/":
        	        ...
        	        self.end_headers()
			fileopen=file("%s/src/webattack/web_clone/site/template/index.html" % (definepath), "r")
			for line in fileopen:
		...
		# used for index2
                if self.path == "/index2.html":
                        ...
                        self.send_header('Content_type', 'text/html')
                        self.end_headers()
                        fileopen=file("%s/src/webattack/web_clone/site/template/index2.html" % (definepath), "r")
                        for line in fileopen:
                                self.wfile.write(line)
                        # write out that we had a visit
                        visits.write("hit\n")
		...
		if counter == 0:
			if os.path.isfile("%s/src/webattack/web_clone/site/template%s" % (definepath,self.path)):
				fileopen=file("%s/src/webattack/web_clone/site/template%s" % (definepath,self.path), "rb")
				for line in fileopen:
					self.wfile.write(line)
				fileopen.close()
	...
	def do_POST(self):
		...
		# needed for command center
		if command_center == "on":
			os.chdir(homepath)
			commandwrite=file("src/program_junk/full_query","w")
			commandwrite.write(url)
			commandwrite.close()	
		...
		os.chdir(homepath+"/src/webattack/web_clone/site/template")
...
if ssl_flag == 'true':
	...
	#if self_signed =="true":
	subprocess.Popen("cp src/program_junk/*.pem src/webattack/web_clone/site/template/ 1> /dev/null 2> /dev/null", shell=True).wait()

# head over to cloned dir
os.chdir("src/webattack/web_clone/site/template/")
