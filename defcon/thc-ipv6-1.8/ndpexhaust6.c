...
//  printf("Sending ICMP ECHO to %s\n", ip6adr);
  ...
  if (thc_add_icmp6(pkt, &pkt_len, ICMP6_PINGREQUEST, 0, 0x34567890, buf, sizeof(buf), 0) == -1)
  ...
  if (thc_generate_and_send_pkt(interface, srcmac, dstmac, pkt, &pkt_len) < 0)
    err(EXIT_FAILURE, "Error sending packet no. %d on interface %s: ", count, interface);
