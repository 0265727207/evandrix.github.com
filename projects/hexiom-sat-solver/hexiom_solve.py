...
# c = associated vertex
# d = index on the side (0 is the vertex, r-1 is the latest)
#
#  |0__ |1
#  /      \
# 5        --
...
# Positional coordinates, as they appear in input:
#
#  0 1
# 2 3 4
#  5 6
...
def SAT_formulation_from_board_input(board_input):
    ...
    print 'Adding Symmetry-breaking rules...'

    def Symmetry_is_preserved(xs, ys):
        xys = zip(xs, ys)

        for m in board_input.blocked_positions:
            m_ = assoc_get(m, xys)
            if m_ not in board_input.blocked_positions:
                #print m, 'to', m_, 'Symmetry not found'
                return False

        for (m,v) in board_input.fixed_positions:
            m_ = assoc_get(m, xys)
            if (m_,v) not in board_input.fixed_positions:
                #print (m,v), 'to', (m_, v), 'Symmetry not found'
                return False
        return True
    
    def vars_from_sym(ms):
        vs = []
        for m in ms:
    ...
    simms = topology.symmetries()
    sim0 = simms[0]
    vsim0 = vars_from_sym(sim0)
    for (i,sim1) in enumerate1( simms[1:] ):
        if Symmetry_is_preserved(sim0, sim1):
            print '  (Symmetry #%s found!)'%(i) 
            vsim1 = vars_from_sym(sim0)
...
def main():
    ...
    print cnf_in_filename, cnf_out_filename
